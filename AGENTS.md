...
11. ~~**ViewPatterns**~~ — DONE (`{-# LANGUAGE ViewPatterns #-}` enables `(expr -> pat)` syntax in patterns; CST parser `scan_for_view_arrow_chirho` detects `->` (both `RightArrowChirho` and `VarSymChirho`) at paren depth 1; `ViewPatChirho` CST node; AST `ViewChirho` pattern variant with view expression + inner pattern; CST→AST lowering handles token kind discrimination for arrow; name resolution recurses into inner pattern; type inference assigns fresh type var; exhaustiveness delegates to inner pattern; desugaring: VarChirho inner → `let n = (view_expr scrutinee) in rhs` (avoids unsupported case binder in STG lowerer); WildcardChirho → case with fresh binder; constructor inner → case with constructor alt using pre-bound binder IDs; `prebind_nested_pat_vars_chirho` used for view pattern inner variables in both all-default and non-default paths; 3 e2e tests: basic `(double -> n)`, ADT `(getVal -> n)`, lambda view expr; 1844 tests)
12. ~~**PatternSynonyms**~~ — DONE (`{-# LANGUAGE PatternSynonyms #-}` enables `pattern Name args = pat` (bidirectional) and `pattern Name args <- pat` (unidirectional) syntax; CST parser `parse_pat_syn_decl_chirho` with `peek_next_is_conid_chirho`; AST `PatSynDeclChirho` variant with `PatSynDirChirho` enum (ImplBidirChirho/UnidirChirho/ExplBidirChirho); CST→AST lowering `lower_pat_syn_decl_chirho`; name resolution binds synonym name in value namespace; interface exports synonyms as values; desugarer: `PatSynDefChirho` collection, `expand_pat_syn_chirho` substitution in `pat_to_alt_con_chirho`/`pat_to_binders_chirho`/`prebind_all_pat_vars_chirho`; bidirectional synonyms usable in expression position via `pat_to_builder_expr_chirho` lambda generation; `substitute_pat_chirho` recursive pattern-level substitution; 3 e2e tests: unidirectional `Head x <- (x:_)`, bidirectional `Pair a b = (a, b)`, AST presence; 1847 tests)
