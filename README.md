...
| GHC `should_compile` (do we **accept** what GHC accepts?) | **849 / 938 (~90%)** — all 89 failures deterministic. Re-measured 2026-07-27 after the determinism fix ([artifact](spec-chirho/ghc-should-compile-measurement-chirho.txt)) |
...
|---------|-----|-------------------|
| Type checking — accepts valid code | Reference | ~90% (849/938 `should_compile`) |
| Type checking — rejects invalid code | Reference | ~19% (149/767 `should_fail`) — accepts 618 programs GHC rejects |
| Same input, same answer twice | Deterministic | **Yes** — fixed 2026-07-27; verified by a 100-run loop ([writeup](spec-chirho/bug-nondeterministic-typecheck-chirho.md)) |
...
- **~~Non-deterministic type checking~~ — FIXED 2026-07-27**: the same file checked twice used to be acceptable on one run and rejected on the next. Root cause was signature schemes quantifying in hash-iteration order; fixed by sorting quantified variables into first-appearance order, verified by a 100-run acceptance loop and a re-measured corpus with a byte-identical failing set. Builds are reproducible. ([writeup](spec-chirho/bug-nondeterministic-typecheck-chirho.md))
