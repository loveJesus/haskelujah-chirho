...
**Lesson:** Runtime performance on non-native platforms requires dedicated effort beyond just getting code to compile. Reusing GHC's frontend accelerates compatibility but creates tight coupling that makes independent evolution difficult. Haskelujah should build its own frontend for long-term independence.
...
**Lesson:** Whole-program optimization yields excellent code quality but does not compose well with separate compilation or incremental builds. Haskelujah should use per-module optimization with limited cross-module inlining, reserving whole-program analysis for optional link-time optimization.
