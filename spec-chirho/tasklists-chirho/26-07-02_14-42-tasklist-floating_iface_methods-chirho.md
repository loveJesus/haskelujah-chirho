...
# Floating Interface Methods

- [x] Confirm no active build/test process and claim the Metropoliluya slot.
- [x] Add seeded `Floating` methods to the central interface method normalizer.
- [x] Add compile-only coverage for `Floating(..)` imports.
...
Validation notes:
- `CARGO_BUILD_JOBS=2 cargo test -p haskelujah-driver --lib frontend_normalized_class_dot_imports_expose_floating_methods_chirho -- --test-threads=1`
