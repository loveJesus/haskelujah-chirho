...
- [x] **Lane 6 landed** (b6ae5a89): bounded module search — >60s unbounded walk → 1.13s. Symlinks never followed (free cycle guard), dir budget 2048, depth 64, file budget 16384, non-silent truncation. Both axes byte-identical after. The handoff's *stack overflow* claim did NOT reproduce; the real defect is unbounded **scope**, reproducible only when the checked file's parent has a large subtree.
