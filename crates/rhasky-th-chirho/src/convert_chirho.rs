...
//! # TH-to-AST conversion — lowering TH splice output back to rhasky AST
//!
...
//! module converts those TH types back into `rhasky-ast-chirho` types so
//! they can be inserted into the module's AST and continue through the
//! normal compilation pipeline.

use rhasky_ast_chirho::decl_chirho::{
...
use rhasky_ast_chirho::ty_chirho::{ConstraintChirho, TypeChirho};
...
/// Convert a TH name to a rhasky AST name.
...
/// Convert a TH type to a rhasky AST type.
...
/// Convert a TH pattern to a rhasky AST pattern.
...
/// Convert a TH literal to a rhasky AST literal.
...
/// Convert a TH expression to a rhasky AST expression.
...
/// Convert a TH match to a rhasky case alternative.
...
/// Convert a TH body to a rhasky RHS.
            ...
            RhsChirho::GuardedChirho(
                guards_chirho
                    ...
                    .map(|(g_chirho, e_chirho)| {
                        ...
                        rhasky_ast_chirho::expr_chirho::GuardedExprChirho {
...
/// Convert a TH statement to a rhasky statement.
...
/// Convert a TH declaration to a rhasky top-level declaration.
...
/// Convert a TH constructor to a rhasky constructor declaration.
