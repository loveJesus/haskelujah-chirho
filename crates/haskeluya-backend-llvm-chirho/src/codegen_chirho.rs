...
impl LlvmCodegenChirho {
    ...
    pub fn compile_module_chirho(&mut self, module_chirho: &CoreModuleChirho) -> String {
        ...
        writeln!(
            self.output_chirho,
            "declare i64 @haskeluya_negate_chirho(i64)"
        )
        .unwrap();
        writeln!(self.output_chirho).unwrap();
                ...
                match callee_chirho {
                    CoreExprChirho::VarChirho(id_chirho) => {
                            ...
                            self.toplevel_names_chirho.get(id_chirho)
                        {
                            ...
                            format!("@haskeluya_v{}", id_chirho.0)
                        };
                        writeln!(
                            self.output_chirho,
...
fn mangle_name_chirho(name_chirho: &str) -> String {
    ...
    mangled_chirho.push_str("haskeluya_");
...
/// point that calls `haskeluya_main()` and prints the i64 result via `printf`.
...
pub fn compile_core_to_llvm_executable_chirho(module_chirho: &CoreModuleChirho) -> String {
    ...
    let filtered_module_chirho =
        haskeluya_core_chirho::elide_dicts_and_filter_chirho(module_chirho);
    ...
    if has_main_chirho {
        ...
        writeln!(ir_chirho, "  %result = call i64 @haskeluya_main()").unwrap();
...
mod tests_chirho {
    use super::*;
    use haskeluya_core_chirho::{BinderChirho, CoreIdChirho, InlineAnnotationChirho};
    use haskeluya_typing_chirho::ty_chirho::TyChirho;
    use haskeluya_core_chirho::CoreBindingChirho;
    ...
    fn compile_simple_constant_chirho() {
        ...
        assert!(ir_chirho.contains("define i64 @haskeluya_main()"));
    ...
    fn compile_identity_function_chirho() {
        ...
        let ir_chirho = compile_core_to_llvm_chirho(&module_chirho);
        assert!(ir_chirho.contains("define i64 @haskeluya_f(i64 %v0)"));
    ...
    fn mangle_name_basic_chirho() {
        assert_eq!(mangle_name_chirho("main"), "haskeluya_main");
        assert_eq!(mangle_name_chirho("f'"), "haskeluya_f_prime");
        assert_eq!(mangle_name_chirho("Data.Map"), "haskeluya_Data_Map");
    }
    ...
    fn compile_executable_with_main_chirho() {
        ...
        // Should contain the haskeluya_main function
        assert!(ir_chirho.contains("define i64 @haskeluya_main()"));
        // Should contain the C main entry point
        assert!(ir_chirho.contains("define i32 @main()"));
        assert!(ir_chirho.contains("call i64 @haskeluya_main()"));
    ...
    fn compile_executable_without_main_chirho() {
        ...
        assert!(!ir_chirho.contains("define i64 @haskeluya_helper()"));
