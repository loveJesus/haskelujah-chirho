...
use haskeluya_backend_cranelift_chirho::{
    compile_core_to_object_executable_chirho, TargetConfigChirho,
};
use haskeluya_backend_llvm_chirho::compile_core_to_llvm_executable_chirho;
use haskeluya_backend_wasm_chirho::compile_core_to_wasm_executable_chirho;
use haskeluya_driver_chirho::{
    compile_source_chirho, eval_source_with_machine_chirho,
    render_diagnostics_chirho, render_summary_chirho,
};
use haskeluya_runtime_chirho::ExecutionModeChirho;
...
fn main_chirho() -> ExitCode {
    ...
    let program_name_chirho = raw_args_chirho
        .first()
        .map(String::as_str)
        .unwrap_or("haskeluya-cli-chirho");
        ...
        "check" | "plan" | "script" => {
...

            {
                ...
                let source_file_result_chirho =
                    haskeluya_syntax_chirho::SourceFileChirho::from_path_with_map_chirho(
                        &mut sm_chirho,
                        &path_chirho,
                    );
                match source_file_result_chirho {
                    Ok(source_file_chirho) => {
                        match haskeluya_driver_chirho::check_source_file_chirho(
                            source_file_chirho,
...
/// `haskeluya run <file.hs>` — evaluate a Haskell program through the STG machine
        ...
        match compile_source_chirho(&source_text_chirho, &mut sm_chirho, file_name_chirho) {
            Ok(result_chirho) => {
                if flags_chirho.dump_core_chirho {
                    ...
                    eprintln!(
                        "{}",
                        haskeluya_core_chirho::pretty_module_chirho(&result_chirho.core_chirho)
                    );
                }
...
/// `haskeluya compile <file.hs>` — run the full compilation pipeline and report
...
/// `haskeluya build [<dir>]` — compile a multi-module Haskell project from a directory.
///
...
fn build_command_chirho(
    ...
    if let Some(cabal_path_chirho) = cabal_file_chirho {
        ...
        let index_chirho = haskeluya_package_chirho::PackageIndexChirho::new_chirho();
        match haskeluya_driver_chirho::compile_cabal_project_chirho(&cabal_path_chirho, &index_chirho) {
            Ok(result_chirho) => {
                eprintln!(
        ...
        let mut sm_chirho = SourceMapChirho::new_chirho();
        match haskeluya_driver_chirho::compile_project_dir_chirho(project_path_chirho, &mut sm_chirho) {
            Ok(result_chirho) => {
                eprintln!(
...
/// `haskeluya install <package-name> <version>` — fetch a package from Hackage,
...
fn install_command_chirho(
    ...
    let version_chirho = match haskeluya_package_chirho::parse_version_chirho(version_str_chirho) {
        Some(v_chirho) => v_chirho,
        None => {
    ...
    let install_dir_chirho = std::path::PathBuf::from(".haskeluya-packages-chirho");
    ...
    let mut db_chirho = if db_path_chirho.exists() {
        match fs::read_to_string(&db_path_chirho) {
            Ok(content_chirho) => {
                haskeluya_package_chirho::InstalledPkgDbChirho::from_string_chirho(&content_chirho)
            }
            Err(_) => haskeluya_package_chirho::InstalledPkgDbChirho::new_chirho(),
        }
    } else {
        haskeluya_package_chirho::InstalledPkgDbChirho::new_chirho()
    };

    // Empty index for now — real Hackage index would be populated separately.
    let index_chirho = haskeluya_package_chirho::PackageIndexChirho::new_chirho();

    match haskeluya_driver_chirho::install_package_chirho(
        pkg_name_chirho,
        &version_chirho,
        &install_dir_chirho,
        &index_chirho,
