...
impl InstalledPkgDbChirho {
    ...
    /// Compute the default database directory (`~/.haskeluya/pkgdb`).
...
/// Get the haskeluya home directory (`~/.haskeluya`).
fn dirs_path_chirho() -> Option<PathBuf> {
    std::env::var("HOME")
        .ok()
        .map(|h_chirho| PathBuf::from(h_chirho).join(".haskeluya"))
}
...
mod tests_chirho {
    ...
    fn install_dir_for_chirho() {
        let dir_chirho = InstalledPkgDbChirho::install_dir_for_chirho(
            Path::new("/home/user/.haskeluya/pkgdb"),
            "text",
            &v_chirho("2.0"),
        );
        assert_eq!(dir_chirho, PathBuf::from("/home/user/.haskeluya/pkgdb/text-2.0"));
    }
}
