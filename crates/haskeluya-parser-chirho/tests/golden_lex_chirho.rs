// For God so loved the world that he gave his only begotten Son, that whoever
// believes in him should not perish but have eternal life. — John 3:16

//! Golden tests for the raw lexer.
//!
//! Each `.hs` file under `tests-chirho/golden-chirho/lex-chirho/` is lexed and
//! the token stream is compared against a sibling `.expected` file.
//! Run with `BLESS_CHIRHO=1` to regenerate expected files.

use haskeluya_parser_chirho::lexer_chirho::LexerChirho;
use haskeluya_span_chirho::FileIdChirho;
use haskeluya_test_harness_chirho::{
    assert_golden_chirho, bless_golden_chirho, discover_golden_tests_chirho,
};
use std::path::PathBuf;

fn golden_dir_chirho() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../tests-chirho/golden-chirho/lex-chirho")
}

fn render_tokens_chirho(source_chirho: &str) -> String {
    let file_id_chirho = FileIdChirho::SYNTHETIC_CHIRHO;
    let mut lexer_chirho = LexerChirho::new_chirho(source_chirho, file_id_chirho);
    let tokens_chirho = lexer_chirho.lex_all_chirho();
    let mut output_chirho = String::new();
    for tok_chirho in &tokens_chirho {
        let start_chirho = tok_chirho.span_chirho.start_chirho().as_usize_chirho();
        let end_chirho = tok_chirho.span_chirho.end_chirho().as_usize_chirho();
        let text_chirho = &source_chirho[start_chirho..end_chirho];
        let display_text_chirho = text_chirho
            .replace('\n', "\\n")
            .replace('\r', "\\r")
            .replace('\t', "\\t");
        output_chirho.push_str(&format!(
            "{:>4}..{:<4} {:?} {:?}\n",
            start_chirho, end_chirho, tok_chirho.kind_chirho, display_text_chirho,
        ));
    }
    output_chirho
}

#[test]
fn golden_lex_all_chirho() {
    let bless_chirho = std::env::var("BLESS_CHIRHO").is_ok();
    let dir_chirho = golden_dir_chirho();
    let cases_chirho =
        discover_golden_tests_chirho(&dir_chirho).expect("should discover golden lex tests");

    assert!(
        !cases_chirho.is_empty(),
        "No golden lex tests found in {}",
        dir_chirho.display()
    );

    let mut failures_chirho: Vec<String> = Vec::new();

    for case_chirho in &cases_chirho {
        let source_chirho =
            std::fs::read_to_string(&case_chirho.input_path_chirho).expect("read .hs file");
        let actual_chirho = render_tokens_chirho(&source_chirho);

        if bless_chirho {
            bless_golden_chirho(&actual_chirho, &case_chirho.expected_path_chirho)
                .expect("bless golden");
        } else if let Err(mismatch_chirho) =
            assert_golden_chirho(&actual_chirho, &case_chirho.expected_path_chirho)
        {
            failures_chirho.push(format!(
                "--- {} ---\n{}",
                case_chirho.name_chirho, mismatch_chirho
            ));
        }
    }

    if !failures_chirho.is_empty() {
        panic!(
            "{} golden lex test(s) failed:\n\n{}",
            failures_chirho.len(),
            failures_chirho.join("\n\n")
        );
    }
}
