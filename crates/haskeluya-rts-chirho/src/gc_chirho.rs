...
//! to find all reachable closures, then replaces dead closures with tombstones
//! that can be reused by future allocations.
//!
//! ## Design
//!
...
mod tests_chirho {
    ...
    fn notify_alloc_triggers_at_threshold_chirho() {
        ...
        assert!(!gc_chirho.notify_alloc_chirho());
        assert!(!gc_chirho.notify_alloc_chirho());
        assert!(gc_chirho.notify_alloc_chirho());
    }
    ...
    #[test]
    fn extract_roots_from_values_chirho_test() {
    ...
    fn transitive_reachability_chirho() {
        ...
        assert_eq!(stats_chirho.live_count_chirho, 3);
        assert_eq!(stats_chirho.dead_count_chirho, 1);
    }
