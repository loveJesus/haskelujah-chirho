...
mod symbols_chirho;
pub(crate) use symbols_chirho::is_symbol_char_chirho;
...
impl<'src> LexerChirho<'src> {
    ...
    pub fn next_token_chirho(&mut self) -> RawTokenChirho {
        ...
        match byte_chirho {
            ...
            b'-' => {
                if self.starts_line_comment_chirho() {
            ...
            // Template Haskell closing quotes, otherwise an ordinary operator.
            b'|' => {
            ...
            _ if self.has_more_symbol_at_chirho(0) => self.lex_operator_chirho(start_chirho),

            // Non-ASCII bytes — could be Unicode identifiers, BOM, etc.
            _ if byte_chirho > 0x7F => {
                // Lambda is a letter, not a Unicode symbol. Preserve the existing
                // UnicodeSyntax spelling; symbol punctuation is handled above.
                if self
                    .source_chirho
                    .get(self.pos_chirho..)
                    .is_some_and(|rest_chirho| rest_chirho.starts_with('λ'))
                {
                    self.pos_chirho += 'λ'.len_utf8();
                    return self
                        .make_token_chirho(RawTokenKindChirho::BackslashChirho, start_chirho);
                }
                ...
                if let Some(ch_chirho) = rest_chirho.chars().next() {
                    if ch_chirho.is_uppercase() {
                        self.lex_upper_ident_chirho(start_chirho)
                    } else if ch_chirho.is_alphabetic() || ch_chirho == '\u{FEFF}' {
                        ...
                        // Non-symbol marks/control characters are not invented operators.
                        self.pos_chirho += ch_chirho.len_utf8();
