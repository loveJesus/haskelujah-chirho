...
//! # haskelujah-backend-wasm-chirho
//!
//! WebAssembly backend for the Haskelujah compiler. Translates Core IR into
...
pub fn compile_to_wasm_stub_chirho(module_name_chirho: &str) -> Vec<u8> {
    format!("haskelujah-wasm-stub:{module_name_chirho}").into_bytes()
}
