...
use haskelujah_core_chirho::expr_chirho::CoreModuleChirho;
...
pub fn jit_execute_chirho(module_chirho: &CoreModuleChirho) -> Result<i64, String> {
    let _filtered_chirho = haskelujah_core_chirho::elide_dicts_and_filter_chirho(module_chirho);

    // Build JIT module
    let mut flag_builder_chirho = cl_settings_chirho::builder();
    cranelift_codegen::settings::Configurable::set(&mut flag_builder_chirho, "is_pic", "false")
        .map_err(|e_chirho| format!("JIT settings: {e_chirho}"))?;

    let isa_builder_chirho =
        cranelift_codegen::isa::lookup_by_name(&target_lexicon::Triple::host().to_string())
    ...
    let mut jit_builder_chirho =
        JitBuilderChirho::with_isa(isa_chirho, cranelift_module::default_libcall_names());
    ...
    jit_builder_chirho.symbol("haskelujah_alloc_chirho", jit_alloc_chirho as *const u8);

    let _jit_module_chirho = JitModuleChirho::new(jit_builder_chirho);
